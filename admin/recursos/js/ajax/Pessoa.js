function inserirPessoa() {
    if ($("#nomeFPessoa").val() == null || $("#nomeFPessoa").val() == "") {
        swal("Atenção", 'Por favor preencha nome!', "error");
    } else if (document.fpessoa.email.value == null || document.fpessoa.email.value == "") {
        swal("Atenção", 'Por favor preencha e-mail!', "error");
    } else if ($("#senhaFPessoa").val() == null || $("#senhaFPessoa").val() == "") {
        swal("Atenção", 'Por favor preencha senha!', "error");
    } else if ($("#senha2FPessoa").val() == null || $("#senha2FPessoa").val() == "") {
        swal("Atenção", 'Por favor preencha repetir senha!', "error");
    } else if ($("#senhaFPessoa").val() != null && $("#senha2FPessoa").val() != null
            && $("#senhaFPessoa").val() != $("#senha2FPessoa").val()) {
        swal("Atenção", 'Senha e repetir senha estão diferentes!', "error");
    } else {
        $.ajax({
            url: "../control/SalvarPessoa.php",
            type: "POST",
...
function salvarNewsletter() {
    $.ajax({
        ...
        success: function (data, textStatus, jqXHR) {
            if (data.situacao === true) {
                swal("Newsletter salva", data.mensagem, "success");
            } else if (data.situacao === false) {
...
function excluir2Pessoa(codpessoa) {
    swal({
...
function btNovoPessoa() {
    ...
    } else if ($("#categoria").val() != "1" && $("#categoria").val() != "6") {
