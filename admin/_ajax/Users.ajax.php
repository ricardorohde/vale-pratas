            ...
            if ($Read->getResult()):
                ...
                if ($Read->getResult()):
                    ...
                    if (!empty($PostData['user_password'])):
                        if (strlen($PostData['user_password']) >= 5 && strlen($PostData['user_password']) <= 11):
                            $PostData['user_password'] = hash('sha512', $PostData['user_password']);
                        else:
                            $jSON['trigger'] = AjaxErro("<b class='icon-warning'>ERRO DE SENHA:</b> Olá {$_SESSION['userLogin']['user_name']}, a senha deve ter entre 5 e 11 caracteres para ser redefinida!", E_USER_WARNING);
                            echo json_encode($jSON);
                            return;
                        endif;
                    else:
