                ...
                <div class="single_order_items">
                    ...
                    if (!$Read->getResult()):
                        ...
                        foreach ($Read->getResult() as $Item):
                            ...
                            $Read->FullRead("SELECT user_name, user_lastname FROM " . DB_USERS . " WHERE user_id = :user", "user={$Item['idvendedor']}");
                            ...
                            echo "<p>";
                            if ($Item['pdt_id']):
                                echo "<b>" . str_pad($Item['item_id'], 5, 0, STR_PAD_LEFT) . "</b> - <a target='_blanc' href='dashboard.php?wc=products/create&id={$Item['pdt_id']}' title='Ver Produto!'>" . Check::Words($Item['item_name'], 4) . "</a>";
                                $ItemsPrice += $Item['item_amount'] * $Item['item_price'];
                            else:
                                echo "<b>" . str_pad($Item['item_id'], 5, 0, STR_PAD_LEFT) . "</b> - " . Check::Words($Item['item_name'], 4);
                                $ItemExtra += $Item['item_amount'] * $Item['item_price'];
                            endif;
                            echo "<br><b>Vendedor:</b> {$Vendedor}";
                            echo "</p>";
