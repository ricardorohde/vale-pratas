...
$Read->ExeRead(DB_PDT, "WHERE pdt_name = :nm AND (pdt_inventory >= 1 OR pdt_inventory IS NULL) AND pdt_status = 1", "nm={$URL[1]}");
if (!$Read->getResult()):
...
<div class="container produtct_basics">
    <div class="content">
        <div class="produtct_basics_cover">
            ...
            echo "<img id='product-img' title='{$pdt_title}' alt='{$pdt_title}' src='" . BASE . "/uploads/{$pdt_cover}'/>";
            ...
            if ($Read->getResult()):
                ...
                foreach ($Read->getResult() as $PDTIMG):
                    ...
                    echo "<li><a id='product-thumb' title='{$pdt_title} - Foto {$i}' href='" . BASE . "/uploads/{$PDTIMG['image']}'><img title='{$pdt_title} - Foto {$i}' alt='{$pdt_title} - Foto {$i}' src='" . BASE . "/tim.php?src=uploads/{$PDTIMG['image']}&w=" . THUMB_W / 3 . "&h=" . THUMB_H / 3 . "'/></a></li>";
                endforeach;
                echo "</ul>";
            endif;
            ?>
            ...
            <div class="info">
                ...
                <div class="price">
                    ...
                    if (ECOMMERCE_PAY_SPLIT):
                        ...
                        echo "<p class='price_split'>Em até {$NumSplit}x de R$ {$SplitPrice}</p>";
                    endif;
                    ?>
                </div>
            </div>
...
<?php
$Read->ExeRead(DB_PDT, "WHERE (pdt_inventory >= 1 OR pdt_inventory IS NULL) AND pdt_status = 1 AND pdt_offer_start <= NOW() AND pdt_offer_end >= NOW() ORDER BY RAND() LIMIT 4");
if ($Read->getResult()):
    ?>
